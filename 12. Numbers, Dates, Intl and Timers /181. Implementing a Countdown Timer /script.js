...
btnLogin.addEventListener('click', function (e) {
  ...
  if (currentUser && currentUser.pin === Number(pin)) {
    ...
    // Add the timer functionality here
    // Also, add functionality which checks if an exsting timer is running or not.
    // If yes, the timer is first cleared.
...
// ADDING LOGOUT TIMER TO APPLICATION

// This timer will call the callback function after every one second.
const logOutTimer = function () {
  ...
  const tick = function () {
    ...
    // Now, we want that when time becomes zero, the timer stops and the user logs out
    if (time === 0) {
      clearInterval(timer);
  ...
  // Called initially when the user is logged in to refresh the timer instatntly so that it starts from initial timimg.
  tick();

  // After that, the timer starts.
  timer = setInterval(tick, 1000);

  // When timer becomes zero, the timer is deleted and the user is logged out and all the transactions are cleared.
};
...
// Implemeting Countdown Timers
// We want to add a countdown timer to the bottom of the screen
// We can use setInterval for that.
