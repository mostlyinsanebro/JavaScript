...
btnLoan.addEventListener('click', function (e) {
  ...
  if (amount > 0 && currentUser.movements.some(mov => mov >= amount * 0.1)) {
    // added new code here to to wait for 2.5 seconds before adding loan amount to currentAccount
    setTimeout(function () {
...
// setTimeOut timer runs only once after a specified time have elapsed.

// setInterval timer keeps on running forever until we stop it. It runs after the
// intervals of specified time.

// setTimeOut Timer
// setTimeOut function takes the callback function as the first argument and the time after which the
// setTimeOut function will call the callback function as the second argument.

setTimeout(() => console.log('Here is your pizza'), 3000);
// This function will wait for three seconds after the code has been compiled to call
// the callBack function.

// The code execution does not simply wait and stop at the setTimeOut function for it to call
// the callback functions. What happens is, when the code execution reaches the setTimeOut function
// then js simply registers the callback function to be called later and keeps counting the time in the background
// and the code execution simply continues and goes to the next line.
// The callback function is called after the specified time has elapsed.
// This mecanism is called Aysnchronous js.
console.log('Waiting');

// PASSING ARGUMENTS
// to the callback function of the setTimeOut function.
// We can pass the argumenst to the callback function of setTimeOut by passing them after
// the delay will be arguments to the callback function.
...
setTimeout(
  ...
  'olives',
  'spinach'
); // The two arguments after the delay can be passed to callback function as arguments.

// CANCELLING TIMEOUT
// We can also delete out the timer which will result in the callback function not being called using the
// clearTimeOut function. It takes the timer variable as an argument.
const ingridients = ['olive', ''];
const pizzaTimer = setTimeout(
  (ing1, ing2) => console.log(`Here is your pizza with ${ing1}`),
  3000,
  ...ingridients
);
...
// This is all you need to know regarding setTimeOut function for now.
// BNow, add functionality to add newly approved loan amount from the bank
// to the transactions using setTimeOut  function.

// setTimeOut function simply schedules the callback function to run only once after the
// specified time has elapsed.

// setInterval function schedules the callback function to run again and again after the interval
// of specified time till eternity or until we stop it.

// Print a clock to the console
setInterval(() => {
  const now = new Date();
