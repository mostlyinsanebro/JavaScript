...
// Chaining Methods
// We can chain the map,filter and reduce methods in order to get the
// result writing very less code that is very concise.

const EURtoUSD = 1.1;

//PIPELINE
// The below code acts as a pipeline for our arrays.
// We pass an array at the start of a pipeline and some actions are performed
// on that data and then it comes out of the pipeline transformed.

// We use the reduce method at the end of this chaining as the array returned from
// one method acts as an input to the next method but reduce does not return an array, so it is used at last.
// The result at the end will be saved in the convertEURtoUSED constant.

const convertEURtoUSD = movements
