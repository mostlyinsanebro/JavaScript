...
const depositTotal = accounts
  ...
  .filter((mov) => mov > 0)
  .reduce((acc, mov) => acc + mov, 0);
console.log(`Total deposited amount in the bank is ${depositTotal}.`);

// 2. How many deposits have been in the bank with atleast 1000 dollars.
const totalBigDeposits = accounts
  .flatMap((acc) => acc.movements)
  .reduce((count, mov) => (mov >= 1000 ? count + 1 : count), 0);

console.log(totalBigDeposits);

// 3. Return an object from the reduce method having deposists and withdrawls in it.
...
// 4.Take a string as Input and convert it to TitleCase leaving a few words out
// like a, an, and , the , but , or ,in, with.

const convertTitleCase = function (title) {
  const capitalize = (title) => title[0].toUpperCase() + title.slice(1);
  const exceptions = ["a", "an", "and", "the", "but", "or", "in", "with", "of"];
...
console.log(convertTitleCase("an exapmle of converting TITLE to titleCase"));
