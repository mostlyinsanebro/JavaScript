...
// Reduce method is used to reduce all the values of an array into one.
// It accumulates all the values of an array into one value and that value is
// finally returned from the method.

// SNOWBALL EFFECT

// SYNTAX
// arrayName.reduce(function callbackFunction(accumulator,currentValue,idx,arr){

// },startvalueofAccumulator);

const sum = movements.reduce(function (acc, curr, i) {
  return acc + curr;
}, 0);
...
for (const mov of movements) sum2 += mov;

console.log(sum2);

// We can also use the reduce method to get a string or object or get the
// maximum value of array.

const maxMovement = movements.reduce(function (acc, val) {
  if (val > acc) return val;
  else return acc;
}, movements[0]);

console.log(maxMovement);
// We are calling the reduce method in this way.
