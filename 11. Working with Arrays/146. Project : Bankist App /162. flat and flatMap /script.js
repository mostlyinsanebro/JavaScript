...
// flat method is basically used to remove the nested arrays from inside arrays
// and put all the elements in an array. It can also go deep into levels as per the
// parameters.

const arr = [[1, 2, 3], [4, 5, 6], 7, 8];
console.log(arr.flat()); // this will flatten the array and put all elements in the same array.

const arrDeep = [[[1, 2], 3], [[4, 5], 6], 7, 8];
console.log(arrDeep.flat()); // This will remove only one-level of arrays.

// In order to make this array really flat, we can go deep into level 2.
console.log(arrDeep.flat(2));

// Now , we want to get the sum of all the movements of all the accounts.

const accountMovements = accounts.map((acc) => acc.movements);

const accountMovementsFlat = accountMovements.flat();

const accountMovementsSum = accountMovementsFlat.reduce(
  (acc, mov) => acc + mov,
  0
);
...
// or we can use flatMap in order to map the array and then flatten it.

const flattenedAndMappedArray = accounts.flatMap((acc) => acc.movements);
console.log(flattenedAndMappedArray.reduce((acc, map) => acc + map, 0));
